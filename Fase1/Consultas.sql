...
SELECT ROW_NUMBER() OVER(ORDER BY T.activo desc) as Posicion, T.banco FROM 
...
SELECT ROW_NUMBER() OVER(ORDER BY T.activo ASC) as Posicion, T.banco, T.activo FROM 
(SELECT b.nombre AS banco, r.activo AS activo, f.fecha AS fecha 
FROM reporte r
JOIN banco b ON b.id = r.banco_id
JOIN periodo f ON f.id = r.fecha_id
...
SELECT ROW_NUMBER() OVER(ORDER BY T.promedio DESC) as Posicion, T.banco FROM 
...
SELECT T.posicion, T.banco FROM(
SELECT DISTINCT 1 as posicion,b.nombre AS banco, AVG(r.activo) OVER (PARTITION BY b.id) AS promedio 
FROM reporte r
JOIN banco b ON b.id = r.banco_id
JOIN periodo f ON f.id = r.fecha_id
